...
from HPOBench.hpobench.dependencies.ml.data_manager import OpenMLDataManager
from HPOBench.hpobench.util.rng_helper import get_rng
...
class MLBenchmark(AbstractBenchmark):
    ...
    def __init__(
        ...
        target_type = type_of_target(self.train_y)
        if target_type in ('binary', 'multiclass'):
            self.train_y = self.train_y.astype(int)
            self.valid_y = self.valid_y.astype(int)
            self.test_y = self.test_y.astype(int)

            le = LabelEncoder()
            # 拿出所有数据，拼接成一个 Series 再 fit
            all_labels = pd.concat([self.train_y, self.valid_y, self.test_y], axis=0)
            le.fit(all_labels)

            # transform 后再转回 Series，并保留原始 index
            self.train_y = pd.Series(le.transform(self.train_y), index=self.train_y.index)
            self.valid_y = pd.Series(le.transform(self.valid_y), index=self.valid_y.index)
            self.test_y = pd.Series(le.transform(self.test_y), index=self.test_y.index)
    ...
    def objective_function(self,
        ...
        return {
            'val_scores': info['val_scores'],
            'cost': model_fit_time + info['val_costs']['acc'],
            # 'info': info
            'info': {'fidelity': info['fidelity']}
        }
