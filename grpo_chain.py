...
class GRPO:
    ...
    def update(self, groups: List[List[Tuple[Dict, Dict, float, float, bool]]]) -> Dict[str, float]:
        ...
        chain_ratio = 0.2  # 轨迹前 20% 作为批外状态
        lambda_chain = 0.01  # CHAIN 正则系数
        ...
        for group in groups:
            ...
            for _ in range(self.epochs):
                ...
                # ===== CHAIN 正则化（轨迹前段作为批外状态） ==========================================
                num_off = max(1, int(len(states) * chain_ratio))
                off_states = states[:num_off]
                off_actions = actions[:num_off]

                with torch.no_grad():
                    old_logp_off = torch.stack([old_policy.get_log_prob(s, a) for s, a in zip(off_states, off_actions)])

                new_logp_off = torch.stack([self.policy.get_log_prob(s, a) for s, a in zip(off_states, off_actions)])

                old_probs = torch.exp(old_logp_off)  # KL
                chain_loss = (old_probs * (old_logp_off - new_logp_off)).mean()

                policy_loss += lambda_chain * chain_loss
                # ==========================================================================================
